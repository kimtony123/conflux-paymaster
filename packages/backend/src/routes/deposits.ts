...
router.post("/verify", async (req: Request, res: Response) => {
  try {
    ...
    const amount = amount_wei ? BigInt(amount_wei) : tx.value;
    await db.createDeposit(dapp.id, amount, tx_hash);
    await db.updateDappBalance(dapp.id, amount);

    console.log(`[Deposits] Verified: ${ethers.formatEther(amount)} CFX for dApp ${dapp.name}`);
